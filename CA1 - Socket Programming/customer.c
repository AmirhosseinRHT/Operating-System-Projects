...
int main(int argc, char *argv[]) 
{
    ...
    struct List restaurants_list = init_names_list();
    ...
    while (1) 
    {
        ...
        else if (FD_ISSET(STDIN_FILENO, &temp_fds)) 
        {
            ...
            if(user.step == INIT)
                user = handle_name_entered(COSTUMER , user , server_fd_udp , server_fd_tcp , broadcast_addr , terminal_buffer);
            ...
            else if(strcmp(splitted[0],"order") == 0 && strcmp(splitted[1] , "food") == 0 && splitted_count == 5)
                order_food(user,restaurants_list,broadcast_addr , splitted[2] ,splitted[3],server_fd_udp ,server_fd_tcp);
            else
